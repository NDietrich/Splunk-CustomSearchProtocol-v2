...
/******************************************************************************
*	averageage.js	Splunk Example Custom Search Command
*
*	calculae the average 'age' field across all events
*	this is a reporting command
*
...
const getinfo_reply = function(){
	...
	try {
		jsonReply = JSON.stringify({
			type 			: 'reporting',		// default is streaming (or stateful, events, reporting)
			required_fields : ['age'],			// could be dynamically determined, defaults to all
...
const modify_payload = function(data){
	...
	events.forEach(function (arrayItem) {
		sumages += parseInt(arrayItem.age)
		numRecords += 1
	});

	log("\tDone processing records. ")

	// build csv from one entry
	avg = sumages / numRecords
	modifiedData = "average\n" + avg + "\n"

	log("average age is " + avg)
