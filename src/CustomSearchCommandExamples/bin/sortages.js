...
/******************************************************************************
*	sortages.js	Splunk Example Custom Search Command
*
*	sorts events based on the value in the 'age' field for all events
*	this is an 'events' command
*
...
const getinfo_reply = function(){
	...
	try {
		jsonReply = JSON.stringify({
			type 			: 'events',		// default is streaming (or stateful, events, reporting)
...
const modify_payload = function(data){
	...
	// in place sort
	events.sort((a,b) => (a.age > b.age) ? 1 : ((b.age > a.age) ? -1 : 0))

	log("\tDone processing records. " + events.length +" records returned.")
